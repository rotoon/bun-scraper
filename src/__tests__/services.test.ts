...
const mockDatabaseModule = {
  getMatches: mock(
    (limit?: number, status?: string, league?: string, _offset?: number) => {
...
describe('FootballService - Advanced Filtering', () => {
  ...
  describe('getMatches with filters', () => {
    ...
    it('should filter by league', async () => {
      ...
      expect(
        result.matches.every(match => match.league === 'Premier League'),
      ).toBe(true);
    ...
    it('should sort by date in ascending order', async () => {
      ...
      const dates = result.matches.map(match =>
        new Date(`${match.matchDate} ${match.matchTime}`).getTime(),
      );
