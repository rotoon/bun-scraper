...
async function runCronJob() {
  ...
  if (now - lastCronRun < MIN_CRON_INTERVAL) {
    console.log(`⏭️ Skipping cron job - too soon since last run (${Math.round((now - lastCronRun) / 60000)} min ago)`)
    return
  }
  ...
  if (cronStatus.consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
    console.log(`🚫 Too many consecutive failures (${cronStatus.consecutiveFailures}), skipping next runs`)
  ...
  try {
    ...
    console.log(`📈 Current stats: ${cronStatus.successfulRuns}/${cronStatus.totalRuns} successful (${Math.round(cronStatus.successfulRuns / cronStatus.totalRuns * 100)}%)`)
...
function logCronStatus() {
  ...
  console.log(`   Last run: ${cronStatus.lastRun ? cronStatus.lastRun.toISOString() : 'Never'}`)
  console.log(`   Last success: ${cronStatus.lastSuccess}`)
  console.log(`   Consecutive failures: ${cronStatus.consecutiveFailures}`)
  console.log(`   Success rate: ${cronStatus.totalRuns > 0 ? Math.round(cronStatus.successfulRuns / cronStatus.totalRuns * 100) : 0}% (${cronStatus.successfulRuns}/${cronStatus.totalRuns})`)
...
function startCronJob() {
  console.log(`⏰ Starting production-ready cron job every ${CRON_INTERVAL / 60000} minutes...`)
  console.log(`🛡️ Rate limiting: minimum ${MIN_CRON_INTERVAL / 60000} minutes between runs`)
  console.log(`🚨 Failure threshold: ${MAX_CONSECUTIVE_FAILURES} consecutive failures`)
