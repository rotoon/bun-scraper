...
describe('CacheManager', () => {
  ...
  describe('Cache Info Building', () => {
    it('should build cache info with all parameters', () => {
      ...
      const cacheInfo = cacheManager.buildCacheInfo(etag, ttl, rateLimitRemaining)

      expect(cacheInfo.etag).toBe(etag)
      expect(cacheInfo.cacheControl).toBe('public, max-age=300, must-revalidate')
    ...
    it('should build minimal cache info', () => {
      ...
      expect(cacheInfo.cacheControl).toBe('public, max-age=60, must-revalidate')
