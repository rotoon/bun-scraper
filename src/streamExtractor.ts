...
export async function getIframeUrl(
  ...
  try {
    ...
    return {
      ...
      message: error instanceof Error ? error.message : "Please provide a valid match ID"
    }
  }
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.error(`❌ Attempt ${attempt} failed for matchId ${matchId}:`, lastError.message)
 ...
 */
export async function checkStreamAvailability(matchId: string): Promise<boolean> {
  try {
    const result = await getIframeUrl(matchId, { timeout: 5000, maxRetries: 1 })
