...
export class StreamController {
  async handleGetStream(matchId: string, request: Request): Promise<Response> {
    ...
    try {
      ...
      const result = await footballService.getMatchStream(
        matchId,
        {},
        clientIP,
      );
      ...
      if (result.success) {
        ...
        if (result.error === 'Rate limit exceeded') {
          ...
          return internalServerErrorResponse(
            result.message || 'Failed to fetch stream',
          );
        }
      }
    } catch (error) {
  ...
  async handleGetBatchStreams(request: Request): Promise<Response> {
    try {
      ...
      return successResponse(
        {
          results,
          summary: { total: results.length, successful: successCount },
        },
        message,
      );
    } catch (error) {
      ...
      return badRequestResponse(
        'Invalid request body or failed to process streams',
      );
    }
  }
}
